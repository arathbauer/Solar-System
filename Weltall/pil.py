...
# Rotations for cube. 
xrot = yrot = zrot = xrot2 = yrot2 = zrot2 = 0.0
...
def set_up_light(name, position=[1, 1, 0, 0], color=[1, 1, 0, 0]):
...
def InitGL(Width, Height):            # We call this right after our OpenGL window is created.
   ...
   # glEnable(GL_COLOR_MATERIAL)
   glLightfv(GL_LIGHT0, GL_AMBIENT, (1, 0, 31, 1.0))		# Setup The Ambient Light
   glLightfv(GL_LIGHT0, GL_DIFFUSE, (0.0, 1.0, 0.0, 1.0))		# Setup The Diffuse Light
   glLightfv(GL_LIGHT0, GL_POSITION, (10.0, 0.0, 0.0, 0.0))	# Position The Light
...
def DrawGLScene():
   global xrot, yrot, zrot, xrot2, yrot2, zrot2, textures, texture_num, quadratic, light
   ...
   glTranslatef(0.0,2.0,0)          # Move Into The Screen
   # axis = (0, 1, 0)
...
def keyPressed(key, x, y):
   global xrot, yrot, zrot, xrot2, yrot2, zrot2, object, texture_num, light
   #
   key = key.lower()
   if key == ESCAPE:
      sys.exit()
      ...
      light = not light
   elif key == b't': #  switch the texture
